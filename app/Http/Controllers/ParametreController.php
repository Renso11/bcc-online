...
{  
    ...
    public function fraisEdit(Request $request){
        try {
            ...
            $frai->type_commission_partenaire = $request->type_partenaire;
            $frai->value_commission_partenaire = $request->value_partenaire;
            $frai->save();
            ...
            foreach($compteCommissions as $item){
                if(array_key_exists('id_'.strtolower($item->libelle),$request->all())){
                    $fc = FraiCompteCommission::where('frai_id',$frai->id)->where('compte_commission_id',$item->id)->first();

                    $fc->type = $request->all()['type_'.strtolower($item->libelle)];
                    $fc->value = $request->all()['value_'.strtolower($item->libelle)];
                    $fc->save();
                }
            }
    ...
    public function permissionsAdd(Request $request){
        try {
            ...
            return redirect()->route('admin.permissions')->withSuccess("Permission enregistré avec succès");
        } catch (\Exception $e) {
            return  back()->withError($e->getMessage());
        };
    }
