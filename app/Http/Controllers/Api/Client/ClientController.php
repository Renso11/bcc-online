...
class ClientController extends Controller
{
    ...
    public function getQuestionsByPhone(Request $request){
        try {             
            ...
            if($client){
                $questions = PasswordResetQuestionClient::with('userClient')->where('user_client_id',$client->id)->where('deleted',0)->orderBy('created_at', 'desc')->take(3)->get()->pluck('password_reset_question');
    ...
    public function confirmKkpTransaction(Request $request, PaiementService $paiementService){
        try{
            ...
            $kkiapay = new \Kkiapay\Kkiapay($public_key,$private_key,$secret, env('KKIAPAY_SANDBOX') == 1 ? true : false);
