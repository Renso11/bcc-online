...
@endsection
@section('title')
    Rapport des transactions par apporteurs
@endsection
@section('page')
    Rapport des transactions par apporteurs
@endsection
            ...
            <div class="col-12">
                <div class="box">
                    <div class="box-header">
                        <h3 class="box-title">Transactions par apporteur </h3>
                    </div>
                    ...
                    <div class="box-body">
                        <form id="form-search">
                            ...
                            <div class="row">
                                <div class="col-md-3">
                                    <label for="">Apporteurs</label>
                                    <select class="form-control select2bs4" id="apporteur" multiple name="apporteurs[]" style="width:100%">
                                        <option value="all">Tous</option>
                                        @foreach ($apporteurs as $apporteur)
                                            <option value="{{$apporteur->id}}">{{$apporteur->lastname.' '.$apporteur->name}}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="col-md-3">
...
@section('js')
    ...
    <script>
        ...
        $('#btn-search').on('click',function(e) {
            ...
            var apporteur = $('#apporteur').val();            

            if(type_operation == "" || status == "" || apporteur == ""){
                toastr.warning("Veuillez renseigner le type d'operation ou le status ou le apporteur")
                $this.html(`<i class="fa fa-search"></i> Rechercher</button>`)
                $this.prop('disabled',false)
            }else{
                $.ajax({
                    url: "/search/transactions/apporteur",
                    data: formData,
                    processData: false,
                    contentType: false,
                    method: "post",
                    ...
                    success: function(data){
                        ...
                        $(".exportToExcel").click(function (e) {
                            ...
                            $($tableClone).table2excel({
                                exclude: ".noExl",
                                name: "Rapport des transactions des apporteurs",
                                filename: "Rapport des transactions des apporteurs.xls",
                                fileext: ".xls",
                                exclude_img: true,
                                exclude_links: true,
                                exclude_inputs: true,
